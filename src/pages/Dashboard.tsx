...
const Dashboard = () => {
  const { user } = useAuth();
  const lostCount = dummyItems.filter((i) => i.type === "lost").length;
  const foundCount = dummyItems.filter((i) => i.type === "found").length;
      ...
      <main className="container py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          ...
          <div className="flex items-center gap-4 rounded-xl border border-border bg-card p-5">
            ...
            <div>
              <p className="text-2xl font-bold text-foreground">{dummyItems.length}</p>
